...
import java.util.stream.*;
 ...
 /**
  * The {@link ObstacleLoader} class is used by {@link ObstacleHandler} to generate
  * the obstacle entities. Firstly, this class tries to load some patterns of obstacles
  * from a file. A pattern is a series of obstacles which all have certain movement 
  * characteristics and a tick associated to them, which defines how long it will take 
  * for the obstacle to be generated after the other of the same pattern.
  * If this procedure fails, this class has a built-in method to generate random patterns
  * of obstacles.
  *
...
public final class ObstacleLoader {
    ...
    /**
     * Defines the minimum number of obstacles which spawns in an instance of a random pattern.
     */
    private static final Integer MIN_OBSTACLES_FOR_INSTANCE = 3;
    /**
     * Defines the maximum number of obstacles which spawns in an instance of a random pattern.
     */
    ...
    /**
     * Defines the range of the Y coordinate an obstacle of a random generate pattern 
    ...
    /**
     * Defines the path of the file which contains the patterns of the obstacles.
     */
    ...
    /**
     * Defines the current tick. Every call of getInstanceOfPattern() increases the tick counter until
     * the pattern of obstacles has been iterated completely, after which returns to 0;
     */
    private Integer tick;
    /*
     * Defines the duration in terms of ticks of the pattern of obstacles that is generated.
     */
    private Integer duration;
    /**
     * Is used to chose the pattern of obstacles. A bigger number means the pattern is more difficult.
     */
    ...
    /**
     * Defines the attributes of the obstacles that are read read from the file which
    ...
    public ObstacleLoader() {
        ...
        /*
         * If the procedure of reading from file fails, a random based generation of obstacle is used.
         */
        try {

            this.in = Objects.requireNonNull(ClassLoader.getSystemResourceAsStream(this.filePath));
            this.readFromFile();
    ...
    private void randomBasedObstacleGeneration() {
        ...
        for (int i = 1; i <= MAX_NUMBER_OF_PATTERNS; i++) {
            ...
            final int numberOfObstacles = MIN_OBSTACLES_FOR_INSTANCE + random.nextInt(MAX_OBSTACLES_FOR_INSTANCE - MIN_OBSTACLES_FOR_INSTANCE);
                    ...
                    switch (typeOfObstacle) {
                        case 0:
                            movement = new Movement.Builder().setPosition(0.0, random.nextDouble(Y_MAP_DIMENSION))
                                    .setSpeed(-(DEFAULT_SPEED_OF_OBSTACLES + i / DEFAULT_SPEED_OF_OBSTACLES), 0.0).build();
    ...
    /**
     * Defines a method used to generate the obstacles associated to the current tick of the pattern.
     * There can be one, more than one, or no obstacle at all corresponding to a certain tick.
     * Every time the method is called the tick counter is increased until it reaches the total duration
