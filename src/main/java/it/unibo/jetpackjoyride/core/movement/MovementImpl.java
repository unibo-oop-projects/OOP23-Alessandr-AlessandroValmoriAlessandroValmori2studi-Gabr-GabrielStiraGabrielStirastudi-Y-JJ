...
public class MovementImpl implements Movement {
    ...
    public MovementImpl(Pair<Double, Double> startingPosition, Pair<Double, Double> startingSpeed, Pair<Double, Double> startingAcceleration, Pair<Double, Double> rotationInfo, List<MovementChangers> listOfChangers) {
    ...
    public void setMovementChangers(List<MovementChangers> listOfChangers) {
        ...
        final Double speedModifier = (this.listOfChangers.contains(MovementChangers.SPEEDY) ? SPEEDYMODIFIER : this.listOfChangers.contains(MovementChangers.SLOW) ? SLOWMODIFIER : 1.0);
        this.speed = new Pair<>(this.speed.get1()*speedModifier, this.speed.get2()*speedModifier); 
        
        final Double accelerationModifier = (this.listOfChangers.contains(MovementChangers.GRAVITY) ? GRAVITYMODIFIER : this.listOfChangers.contains(MovementChangers.INVERSEGRAVITY) ? INVERSEGRAVITYMODIFIER : 0.0);
    ...
    private void applyModifiers(Double sizeX, Double sizeY) {
        ...
        /*if(this.listOfChangers.contains(MovementChangers.HOMING)) {
            if(this.position.get2() != player.getPosY() ) {
                this.acceleration = new Pair<>(this.acceleration.get1(), this.acceleration.get2() + (player.getPosY() - this.position.get2()));
            }
        }*/
